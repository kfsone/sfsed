    ...
    public class Domain
    {
        ...
        public string AsText(bool saving, string indentation="")
        {
            ...
            foreach (var entry in loadOrder)
            {
                if (entry.subDomain != null)
                    text += entry.subDomain.AsText(saving, innerdent);
                else if (entry.property.value != null)
                {
                    text += $"{innerdent}{entry.property.key} = {entry.property.value}\n";
        ...
        public void ListProperties(ListView listView)
        {
            ...
            foreach (Property property in properties)
            {
                ...
                {
                    Tag = property,
                    ForeColor = property.IsChanged ? Color.Red : Form.DefaultForeColor
                };
        ...
        public void ListDomains(TreeNodeCollection insertAt)
        {
            ...
            TreeNode[] treeNodes = new TreeNode[domains.Count];
            ...
            for (int i = 0; i < domains.Count; ++i)
            {
                domains[i].ListDomains(treeNodes[i].Nodes);
            }
        }
    };
}
